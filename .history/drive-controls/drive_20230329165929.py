...
# TODO: Calibrate
def angle_to_duration(angle_degrees, wheel_diameter, wheelbase, speed):
    angle_radians = math.radians(angle_degrees)
    wheel_circumference = math.pi * wheel_diameter
    distance_to_travel = (angle_radians * wheelbase) / 2
    speed_ratio = speed / 100.0
    wheel_speed = wheel_circumference * speed_ratio
    duration = distance_to_travel / wheel_speed
    return duration

def turn_left_angle(speed, angle):
    duration = angle_to_duration(angle, WHEEL_DIAMETER, WHEELBASE, speed)
    turn_left(speed, duration)

def turn_right_angle(speed, angle):
    duration = angle_to_duration(angle, WHEEL_DIAMETER, WHEELBASE, speed)
